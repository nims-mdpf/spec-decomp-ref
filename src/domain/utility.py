...
def initialize(
    data: typing.Dict,
    reference_table: pd.core.frame.DataFrame,
    states: typing.List,
    peaks_obj: domain.model.Signal,
    background_obj: domain.model.Signal,
    noise_obj,
    regularizer_obj,
):
    models = []
    for i, series in enumerate(data.keys()):
        ...
        for j, state in enumerate(states):
            ...
            if len(reference_table_subset.index) == 0:
                ...
                reference_table_subset[RefParamKeys.r.value] = (
                    reference_table_subset[RefParamKeys.r.value] / rsum
                )
                reference_table_subset[RefParamKeys.r.value] = (
                    reference_table_subset[RefParamKeys.r.value]
                    * reference_table_subset[RefParamKeys.A.value]
                )
                reference_table_subset = reference_table_subset[
                    [
                        RefParamKeys.r.value,
                        RefParamKeys.mu.value,
                        RefParamKeys.w.value,
                        RefParamKeys.u.value,
                    ]
                ]
